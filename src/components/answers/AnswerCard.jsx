...
import { ChevronUp, Clock, CheckCircle, XCircle, AlertTriangle, Shield, Trash2, Flag, MessageSquare, Sparkles, MoreVertical } from 'lucide-react'
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <div className="flex justify-between items-start">
          <div className="prose prose-sm dark:prose-invert max-w-none text-slate-800 dark:text-slate-300 leading-relaxed flex-1">
            <p className="whitespace-pre-wrap">{answer.content}</p>
          </div>

          {/* Option Menu (Report Answer) */}
          {user && !isOwner && (
            <div className="relative shrink-0 ml-2">
              ...
              {showMenu && (
                <>
                  ...
                  />
                  <div className="absolute right-0 mt-1 w-36 bg-white dark:bg-slate-800 border border-slate-200/60 dark:border-slate-700/60 rounded-xl shadow-lg py-1 z-40">
                    <button
                      onClick={(e) => {
                        e.stopPropagation();
                        setShowMenu(false);
